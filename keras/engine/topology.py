...
class Container(Layer):
    ...
    def load_weights(self, filepath, map_names=False):
        ...
        if hasattr(self, 'flattened_layers'):
            ...
            flattened_layers = self.layers
        layer_dict = {l.name: l for l in flattened_layers}
        ...
        if 'nb_layers' in f.attrs:
            ...
            if nb_layers != len(flattened_layers):
                if map_names:
                    warnings.warn('Loading weights for %d layers into a model'
                                  'with %d.' % (nb_layers, len(flattened_layers)))
                else:
            ...
            if len(layer_names) != len(flattened_layers):
                if map_names:
                    warnings.warn('Loading weights for %d layers into a model '
                                  'with %d.' % (len(layer_names), len(flattened_layers)))
                else:
                    raise Exception('You are trying to load a weight file '
            ...
            for k, name in enumerate(layer_names):
                ...
                if len(weight_names):
                    ...
                    if map_names:
                        ...
                        if layer.name != name:
                            warnings.warn(
                                'Assigning weights from layer %s to layer %d with name %s.' % (name, k, layer.name))
                    symbolic_weights = layer.trainable_weights + layer.non_trainable_weights
